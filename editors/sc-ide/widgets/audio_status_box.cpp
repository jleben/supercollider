    ...
    m_synthdefs(0)
{
    ...
    connect(server, SIGNAL(stateChanged(int)),
    ...
    onServerStateChanged(ScServer::Inactive);
...
void AudioStatusBox::onServerStateChanged( int state )
{
    bool active = state == ScServer::Active;

    mStatisticsLabel->setTextColor( active ? Qt::green : Qt::white);
    mVolumeLabel->setTextColor( active ? Qt::green : Qt::white );
